...
document.getElementById("saveFiles").addEventListener("click", function() {
  ...
  const file = new Blob([JSON.stringify(flashcardApp.contentArray)]);
    ...
    reader.onload = function (e) {
      try {
        ...
        if (Array.isArray(flashcardsData)) {
          // Assuming the uploaded JSON is an array of flashcards
          flashcardsData.forEach((flashcard) => {
            flashcardApp.contentArray.push(flashcard);
            flashcardApp.flashcardMaker(flashcard, flashcardApp.contentArray.length - 1);
          });

          // Save the updated flashcards data to localStorage
          localStorage.setItem('items', JSON.stringify(flashcardApp.contentArray));

          // Optionally, reload the page or update the UI as needed
          window.location.reload();
        } else {
          alert("Invalid JSON format. Please provide an array of flashcards.");
        }
      } catch (error) {
